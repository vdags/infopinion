...
if __name__=="__main__":
    os.chdir(os.path.abspath(os.path.dirname(__file__)))
    print(os.getcwd())
    df= pd.read_csv("avg_dist_unif.csv")
    mat=time_stab_mat(df)
    print(mat)
    print(df.describe()[["proba_consensus","avg_stab_time","std_stab_time"]])
    ...
    plt.xlabel("opinion_param_1")
    plt.ylabel("msg_param")
    plt.title("Time for convergence (in nb of timesteps)")
    #plt.savefig("avg_time_to_stab_avg_dist_unif.png")
    plt.show()
    ...
    #plt.savefig("std_time_to_stab_avg_dist_unif.png")
    plt.show()
