...
def stabilise(simulation,t=-5,length=5,tol=0.1,ndi=None):
    ...
    for i in range(0,simulation.opinion_size):
        if abs(ndi[i,t]-ndi[0,t+length-1])<=tol*ndi[i,0].mean():
            stab_mat[i,0]=1
        else:
            stab_mat[i,0]=0
    return stab_mat
...
def time_stab(simulation,length=5,tol_stab=0.1,ndi=None):
    ...
    for i in range(0,simulation.opinion_size):
        t=timesteps-length
        while abs(ndi[i,t]-ndi[i,t+length-1])<=tol_stab*ndi[i,0].mean() and t>0:
            t=t-1
        stab_time.append(t)
...
def type_stabilise(simulation,length=5,tol_stab=0.1,tol_pol=0.2,ndi=None,gdi=None):
    """
    ...
    for i in range(0,simulation.opinion_size):
        if abs(ndi[i,t]-ndi[i,t+length-1])<=tol_stab*ndi[i,0].mean():
            if gdi[i,-1]>gdi[i,0]:
                bipolarisation[i]=1
            else:
...
if __name__=="__main__":
    ...
    msg_param_var = "msg_gen_one_info_random_coef"
    ...
    #print(df)
    df.to_csv("avg_rand_unif.csv",index=False)
