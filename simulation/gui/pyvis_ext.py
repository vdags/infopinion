...
class NetworkExt(pyvis.network.Network):
    def from_nx_ext(self, nx_graph, node_size_transf=(lambda x: x), edge_weight_transf=(lambda x: x),
                default_node_size =10, default_edge_weight=1, show_edge_weights=True, edge_scaling=False,hide_edges=False):
        """
        Add 'hide_edges' arg from from_nx pyvis.Network method. 
        ...
        if len(edges) > 0:
            for e in edges:
                ...
                if hide_edges:
                    self.add_edge(e[0], e[1], **e[2],hidden=True)
                elif not hide_edges:
